...
export default async function AdminDashboardPage() {
  ...
  const metricCards = [
    {
      ...
      change: queryErrors.orders ? null : percentChange(current.revenueMinor, previous.revenueMinor),
      icon: PhilippinePeso,
    },
    { label: "Paid orders", value: queryErrors.orders ? "Unavailable" : String(current.orderCount), change: queryErrors.orders ? null : percentChange(current.orderCount, previous.orderCount), icon: ShoppingBag },
    { label: "Average order", value: queryErrors.orders ? "Unavailable" : formatMinorUnitsToPHP(current.averageOrderMinor), change: null, icon: BarChart3 },
    { label: "Items sold", value: queryErrors.orders ? "Unavailable" : String(current.itemsSold), change: null, icon: Package },
  ];
  ...
  return <div className="space-y-6 pb-8">
    ...
    {!queryErrors.orders && current.orderCount < 5 && <div className="flex gap-3 rounded-lg border border-border bg-muted/40 p-4 text-sm text-foreground"><Sparkles className="mt-0.5 size-4 shrink-0 text-muted-foreground" /><p><strong>Limited sales history.</strong> Treat performance signals as early indicators based on the selected period.</p></div>}
    ...
    <section aria-labelledby="attention-title" className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><AlertTriangle className="size-5 text-amber-700" /><h2 id="attention-title" className="admin-section text-foreground">Needs attention</h2></div><div className="flex items-center gap-2 text-xs"><Link href="/admin/orders?status=CONFIRMED" className="rounded-md bg-white/80 px-2 py-1 ring-1 ring-amber-200 hover:bg-white">Confirmed</Link><Link href="/admin/orders?status=PROCESSING" className="rounded-md bg-white/80 px-2 py-1 ring-1 ring-amber-200 hover:bg-white">Processing</Link></div></div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-5">
      <Link href="/admin/payments" className="rounded-xl bg-white p-4 ring-1 ring-amber-200 transition hover:ring-amber-400"><p className="text-2xl font-semibold">{queryErrors.payments ? "—" : pendingCount}</p><p className="mt-1 text-sm font-medium">GCash Verification</p><p className="mt-1 text-xs text-muted-foreground">{queryErrors.payments ? "Queue unavailable" : oldestPending === null ? "No waiting submissions" : `Oldest waiting ${oldestPending}h`}</p></Link>
      <Link href="/admin/orders?status=READY_FOR_SHIPMENT" className="rounded-xl bg-white p-4 ring-1 ring-amber-200 transition hover:ring-amber-400"><p className="text-2xl font-semibold">{queryErrors.readyToShip ? "—" : readyRes.count ?? 0}</p><p className="mt-1 text-sm font-medium">Ready to ship</p><p className="mt-1 text-xs text-muted-foreground">{queryErrors.readyToShip ? "Queue unavailable" : "Open the dispatch queue"}</p></Link>
      <Link href="/admin/returns?status=REQUESTED" className="rounded-xl bg-white p-4 ring-1 ring-amber-200 transition hover:ring-amber-400"><p className="text-2xl font-semibold">{queryErrors.returns ? "—" : returnsCount}</p><p className="mt-1 text-sm font-medium">Returns to inspect</p><p className="mt-1 text-xs text-muted-foreground">{queryErrors.returns ? "Queue unavailable" : returnsCount === 0 ? "All returns inspected" : `${returnsCount} awaiting review`}</p></Link>
      <Link href="/admin/catalog" className="rounded-xl bg-white p-4 ring-1 ring-amber-200 transition hover:ring-amber-400"><p className="text-2xl font-semibold">{queryErrors.inventory ? "—" : lowStock.length + outOfStock.length}</p><p className="mt-1 text-sm font-medium">Inventory risks</p><p className="mt-1 text-xs text-muted-foreground">{queryErrors.inventory ? "Inventory unavailable" : `${outOfStock.length} Out of Stock · ${lowStock.length} Low Stock`}</p></Link>
      <Link href="/admin/orders?status=DELIVERY_FAILED" className="rounded-xl bg-white p-4 ring-1 ring-amber-200 transition hover:ring-amber-400"><p className="text-2xl font-semibold">{queryErrors.deliveryFailures ? "—" : failedRes.count ?? 0}</p><p className="mt-1 text-sm font-medium">Delivery failures</p><p className="mt-1 text-xs text-muted-foreground">{queryErrors.deliveryFailures ? "Queue unavailable" : "Investigate exception orders"}</p></Link>
    </div></section>
